...
const AuthStack_SigninScreen: React.FC<Props> = ({navigation, route}) => {
  ...
  useEffect(() => {
      if (route.params?.passwordRequired)
        setPasswordRequired(route.params?.passwordRequired);
      if (route.params?.countryCode) setCountryCode(route.params?.countryCode);
    }, [route.params]);
  ...
  const onPressSignIn = () => {
    if (phone.length === 0 && !route.params.phoneNumber) {
    ...
    const data = {
      phone: route.params?.phoneNumber || `${countryNumber}${phone}`,
      password,
    };
    setLoading(true);
